		...
		static GUIHelper() {
			...
			if (tyGUIClip != null) {
				...
				if (piVisibleRect != null) {
					MethodInfo info = piVisibleRect.GetGetMethod(true);
                    if (info == null)
						info = piVisibleRect.GetGetMethod(false);
					VisibleRect = (Func<Rect>)Delegate.CreateDelegate(typeof(Func<Rect>), info);
				}
			}
